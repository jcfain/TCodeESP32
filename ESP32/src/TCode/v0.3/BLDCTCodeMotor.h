...
public:
    ...
    { 
        if(encoderType == BLDCEncoderType::MT6701) {
            LogHandler::info(m_TAG, "Selected encoder for %s: MT6701", name);
            if(chipSelectPin > -1) {
            ...
            LogHandler::info(m_TAG, "Selected encoder for %s: PWM", name);
            if(encoderPin > -1) {
    ...
    bool move(int tcode)
    {
        ...
        m_sensor->update();
        float sensorAngle = m_sensor->getAngle();
        // Determine the linear position of the receiver in (0-10000)
        float xPosition = (sensorAngle - zeroAngle)*angToPos; 
